...
@dataclass
class _Settings:
    """Just the two fields profile_exists reads."""
...
def test_directory_without_a_cookie_database_is_not_signed_in(tmp_path):
    ...
    (tmp_path / "x" / "Default").mkdir(parents=True)
    (tmp_path / "x" / "Default" / "Preferences").write_text("{}")
...
def test_empty_cookie_database_is_not_signed_in(tmp_path):
    ...
    """
    _make_cookies(tmp_path / "x" / "Default" / "Cookies", rows=0)

    assert profile_exists(_settings(tmp_path)) is False


def test_cookies_mean_signed_in(tmp_path):
    _make_cookies(tmp_path / "x" / "Default" / "Cookies", rows=11)
...
def test_legacy_network_cookies_path_is_understood(tmp_path):
    ...
    _make_cookies(tmp_path / "x" / "Default" / "Network" / "Cookies", rows=3)
